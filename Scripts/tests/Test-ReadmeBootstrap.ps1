...
if ($bootstrap -cnotmatch '(?m)^  \$tag = ''v2\.1\.2''\r?$') {
    throw 'The README installer bootstrap must pin v2.1.2.'
}
...
try {
    ...
    if (
        ...
        -not ($script:installerArguments -ccontains 'v2.1.2')
    ) {
