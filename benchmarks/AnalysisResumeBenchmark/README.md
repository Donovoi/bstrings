...
Both percentage-only gates are superseded by ADR-0012. The script must be
updated before it can produce acceptance evidence. The corrected protocol uses
at least five alternating pairs and these gates:
...
After the runner implements the corrected protocol, run it from the repository
root:
