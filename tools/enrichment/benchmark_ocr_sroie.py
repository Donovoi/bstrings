...
SROIE_DUPLICATE_IMAGE_POLICY = (
    "train-deduplicate-by-image-and-normalized-annotation-digest-test-score-all-v2"
)
...
def parse_sroie_row(row_index: int, row: Mapping[str, Any]) -> ParsedSroieAnnotation:
    ...
    for index, (source_text, source_box) in enumerate(
        ...
        left, top, right, bottom = _bbox(source_box, index, width, height)
...
def _duplicate_selection(
    ...
    for image_sha256, members in groups.items():
        ...
        if len(ordered) == 1:
            ...
            continue
        annotation_sha256s = sorted({item.annotation_sha256 for item in ordered})
        ...
        duplicate_groups.append(
            {
                ...
                "distinctAnnotationDigests": len(annotation_sha256s),
                "annotationSha256s": annotation_sha256s,
                "decision": decision,
        ...
        for row_index, row in enumerate(source_rows):
            ...
            identities.append(
                _RowIdentity(
                    ...
                    annotation_sha256=sha256_bytes(canonical_json(parsed.document).encode("utf-8")),
                )
            )
